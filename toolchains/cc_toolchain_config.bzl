...
def _impl(ctx):
    ...
    strip_all_symbols_feature = feature(
        name = "strip_all_symbols",
        flag_sets = [
            flag_set(
                actions = all_link_actions + lto_index_actions,
                flag_groups = [flag_group(flags = ["-Wl,-s"])] if "-g" not in ctx.fragments.cpp.copts else [],
                with_features = [with_feature_set(not_features = ["dbg"])],
            ),
        ],
    )

    strip_all_symbols_for_debug_feature = feature(
        name = "strip_all_symbols_for_debug",
        flag_sets = [
            flag_set(
                actions = all_link_actions + lto_index_actions,
    ...
    if is_linux:
        features = [
            ...
            strip_all_symbols_feature,
            strip_all_symbols_for_debug_feature,
            coverage_feature,
