...
cd "$(dirname "$0")"
echo "TZ OTA run in path: ${PWD}"
# 获取当前脚本名称
script_name=$(basename "$0")
echo "TZ OTA execute script: ${script_name}"
...
update_realsense_udev() {
    DEVICE_INFO=$(lsusb | grep "415")

    if [ -z "$DEVICE_INFO" ]; then
        echo "错误：未找到包含 Realsense D415 的 USB 设备"
        echo "错误：不会更新该机器的realsense udev规则"
        return 1
    fi
    ...
    echo "Realsense D415 相机挂载在 Bus${BUS_STR}, 设备号: ${DEV_NUM}"

    case "$BUS_STR" in
        002)
            sudo cp ./udev_realsense/99-usb-cameras.rules.new /etc/udev/rules.d/99-usb-cameras.rules
            ;;
        004|006)
            sudo cp ./udev_realsense/99-usb-cameras.rules.old /etc/udev/rules.d/99-usb-cameras.rules
            ;;
        *)
            echo "[Warning]: 未知 Bus${BUS}，不执行realsense udev规则更新"
            ;;
    esac
...
# 单独更新realsense udev规则
update_realsense_udev


# udev规则刷新
echo "TZ OTA refresh udev rule."
