...
def upgrade_power_board():
    try:
        ...
        if ethercat_map is None:
            ...
            return -1
        ret, is_upgrade, master_id, slave_id, code, bin_name, version = (
            parse_power_board_config(get_mcu_rollback_dir_path() + "/info.yaml")
        )
        if ret != 0:
            logger("无法解析 power_board 配置文件，请检查文件格式是否正确。")
            return -1

        if not is_upgrade and version is None:
            logger("电源板不需要回滚")
            return 0
        ...
        count=1
        while count < retry_count:
            if (update_power_board_version(net_card,master_id[3:],code,get_mcu_rollback_dir_path() + f"/{bin_name}",) != 0):
                logger(f"电源板回滚失败{count}次")
                count +=1
                time.sleep(3)
            else:
                break
...
def upgrade_power_board_by_elink():
    try:
        ...
        ret, is_upgrade,device_addr,bin_name, version = (
            parse_multiplex_link_power_board_config(get_mcu_rollback_dir_path() + "/info.yaml")
        )
        if ret != 0:
            logger("无法解析 power_board 配置文件，请检查文件格式是否正确。")
            return -1
        ...
        count = 1
        while count <= retry_count:
            if (update_power_board_version_by_elink(device_addr, get_mcu_rollback_dir_path() + f"/{bin_name}",) != 0):
                logger(f"电源板回滚失败{count}次")
                count+=1
                time.sleep(3)
            else:
