...
def get_read_xml_tool_path():
    return os.path.join(get_current_script_path(), "../../../bin/hal_ethercat/slaveinfo")
...
def get_mcu_rollback_dir_path():
    return os.path.join(get_current_script_path(), "../../../mcu_rollback")
...
def upgrade_mcus():
    try:
        if not os.path.exists(get_mcu_rollback_dir_path()):
            logger("mcu 回滚目录不存在，因此不对 MCU 回滚")
            return 0
        ...
        for mcu_name in mcu_order:
            ...
            while count <= retry_count:
                ...
                if ret != 0:
                    ...
                    break
            if count >= retry_count:
                logger(f"获取{mcu_name}版本号失败")
                return -1
            install_mcu_version = map_version.get(mcu_name, "")
            if install_mcu_version != "" and install_mcu_version == current_version:
                logger(f"{mcu_name} 的版本号与升级文件一致，无需回滚。")
                continue

            logger(f"已安装版本：{current_version}，回滚版本：{install_mcu_version}")

            logger(f"回滚 {mcu_name}")
            if (not mcu_name.startswith("mcu")):
                logger(f"回滚 {mcu_name}失败")
                return -1

            count = 1
            while count <= retry_count:
                if update_mcu_version(net_card,mcu_name[3:],code,get_mcu_rollback_dir_path() + f"/{bin_name}")!= 0:
                    logger(f"{mcu_name} 回滚失败。信息：{net_card} {code} {mcu_name} {get_mcu_rollback_dir_path()}/{bin_name}")
                    count+=1
                    time.sleep(3)
                else:
                    break
            if count >= retry_count:
                logger(f"{mcu_name}回滚失败")
                return -1

            logger(f"{mcu_name} 回滚成功，睡眠30s")
            time.sleep(30)
...
def upgrade_xml():
    try:
        if not os.path.exists(get_mcu_rollback_dir_path()):
            logger("mcu 回滚目录不存在，因此不对 XML 回滚")
            return 0
        ...
        logger(get_mcu_rollback_dir_path() + "/info.yaml")
        ret, exe_name, xml_map = parse_xml_config(
            get_mcu_rollback_dir_path() + "/info.yaml"
        )
        ...
        if exe_name is None or xml_map is None:
            logger("exe_name 或 xml_map 为空，因此不回滚")
            return 0
        ...
        for key, value in xml_map.items():
            ...
            if net_card is None:
                logger(f"{key} 在 ethercat 配置文件中没有找到对应的网卡，无法回滚。")
                return -1
            net_card = " ".join(net_card)
...
def upgrade_multiple_board_by_elink(mcu_name,bin_name):
    is_ok=True
    if(update_mcu_version_by_elink(mcu_name,get_mcu_rollback_dir_path() + f"/{bin_name}") !=0):
...
def upgrade_mcus_by_elink():
    ...
    if current_model != model:
        logger("不包含P1,不通过elink回滚MCU")
        return 0
    
    logger(f"开始elink回滚MCU.")
    if not os.path.exists(get_mcu_rollback_dir_path()):
        logger("mcu 回滚目录不存在，因此不对 MCU 回滚")
        return 0
    ...
    get_map = parse_multiplex_link_config(get_mcu_rollback_dir_path() + "/info.yaml")
    ...
    for mcu_name in mcu_order:        
        ...
        while count <= retry_count:
            if(upgrade_multiple_board_by_elink(mcu_name,bin_name)!=True):
                logger(f"{mcu_name}回滚失败{count}次")
                count+=1
                time.sleep(3)
            else:
                break
        ...
        time.sleep(5)
        logger(f"{mcu_name} 回滚完成")
    logger("elink mcu回滚完成")
    return 0
